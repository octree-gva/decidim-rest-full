...
RSpec.describe Decidim::Api::RestFull::DraftProposals::DraftProposalsController do
  path "/draft_proposals/{id}/publish" do
    post "Publish a draft proposal (async)" do
      ...
      describe_api_endpoint(
        ...
        let!(:proposal) do
          prop = create(:proposal, published_at: nil, component: proposal_component, users: [user], title: { en: "Title" }, body: { en: "Body" })
  ...
  path "/draft_proposals/{id}/publish/sync" do
    post "Publish a draft proposal (sync)" do
      tags "Draft Proposals"
      produces "application/json"
      security [{ resourceOwnerFlowBearer: ["proposals"] }]
