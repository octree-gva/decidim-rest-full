        ...
        class ProposalsController < Decidim::Api::RestFull::Core::ResourcesController
          ...
          def show
            ...
            render_json_with_conditional_get(
              serialize_proposal_show(@resource),
              fingerprint: Decidim::RestFull::HttpCache::ProposalShowFingerprint.for_request(self, @resource)
            )
          end
          ...
          def serialize_proposal_show(resource)
          ...
          def serialized_show
            serialize_proposal_show(find_proposal!)
          end
          ...
          def collection
            ...
            table = Decidim::Proposals::Proposal.table_name
            ...
            if act_as.nil? || vote_weight_filtered?
              query.where.not("#{table}.published_at" => nil)
            else
              query.where.not("#{table}.published_at" => nil)
                   .or(query.where("#{table}.published_at IS NULL AND decidim_coauthorships.decidim_author_id = ?", act_as.id))
            end
          end
