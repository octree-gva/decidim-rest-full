    ...
    module Test
      module OnApiEndpointMethods
        ...
        def describe_api_endpoint(options = {}, &block)
          ...
          # Bearer auth is documented via OpenAPI securitySchemes (credentialFlowBearer / resourceOwnerFlowBearer).
          # RSwag request specs still need an explicit Authorization header parameter so
          # let(:Authorization) is wired into the Rack request (securitySchemes alone do not).
