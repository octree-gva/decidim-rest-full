...
RSpec.describe "Decidim::Api::RestFull::Proposal::ProposalsController", type: :request do
  path "/public/{space_manifest}/{space_id}/{component_id}/proposals" do
    get "Show a blog detail" do
      ...
      let!(:proposals) { create_list(:proposal, 3, component: proposal_component, published_at: Time.zone.now - 1.day.ago, users: [create(:user, :confirmed, organization: organization)]) }
      let(:"locales[]") { %w(en fr) }
      ...
      response "200", "Proposal Found" do
        ...
        context "when is ordered" do
          ...
          let!(:proposal_id) { Decidim::Proposals::Proposal.where(component: proposal_component).order(published_at: :asc).first.id }
