...
RSpec.describe Decidim::RestFull::Core::MagicToken do
  let(:user) { create(:user) }

  describe "validations" do
    it "is valid with a user, magic_token, and expires_at" do
      magic_token = build(:magic_token, user:)
    ...
    it "is invalid with a duplicate magic_token" do
      create(:magic_token, magic_token: "unique_token", user:)
      magic_token = build(:magic_token, magic_token: "unique_token", user:)
      expect(magic_token).not_to be_valid
  ...
  describe "callbacks" do
    it "sets a URL-safe encoded magic_token before validation" do
      magic_token = build(:magic_token, magic_token: nil, user:)
      magic_token.save
    ...
    it "sets an expires_at before creation" do
      magic_token = build(:magic_token, expires_at: nil, user:)
      magic_token.save
  ...
  describe "#valid_token?" do
    it "returns true if the token is not expired" do
      magic_token = create(:magic_token, expires_at: 1.hour.from_now, user:)
      expect(magic_token.valid_token?).to be true
    end

    it "returns false if the token is expired" do
      magic_token = create(:magic_token, expires_at: 1.hour.ago, user:)
