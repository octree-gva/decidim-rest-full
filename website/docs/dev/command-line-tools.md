...
:::info
Run Ruby and OpenAPI tasks **inside Docker** when developing this module (`docker compose exec rest_full bash -lc 'cd /home/module && …'`). See [First contribution](/contribute/first-contribution).
:::
...
`bin/setup-tests` skips regenerating `spec/decidim_dummy_app` when it looks healthy and the appraisal marker (`.decidim_appraisal`: `decidim-0.29` / `decidim-0.32` / `decidim-0.32-apartment`) matches the current `BUNDLE_GEMFILE` — then it only runs `db:migrate` (no `db:drop`). **Switching Decidim appraisals always regenerates `test_app` and drops the DB** (migrations differ between 0.29 and 0.32; the apartment appraisal also needs tenant migrations). Set `FORCE_SETUP_TESTS=1` to wipe and rebuild within the same appraisal. Optional: `npm_config_maxsockets` (default `2`) and `NODE_OPTIONS` (default `--max-old-space-size=4096`) limit npm memory during `rake test_app`. Commands: [First contribution](/contribute/first-contribution).
