    ...
    module Core
      class SyncronizeUnconfirmedHost < Decidim::Command
        ...
        def call
          ...
          raise Decidim::RestFull::Core::ApiException::BadRequest, "The unconfirmed host cannot be the same as the current host." if organization.host == unconfirmed_host
          ...
          raise Decidim::RestFull::Core::ApiException::NotFound, "Unconfirmed host #{unconfirmed_host} does not resolve." if dns_lookup.empty?
          # For each ip, check if the reverse dns lookup is the unconfirmed_host
          if ips.any? { |ip| dns_lookup.exclude?(ip.to_s) }
            raise Decidim::RestFull::Core::ApiException::NotFound, "Unconfirmed host #{unconfirmed_host} does not resolve to all of the loadbalancer ips."
          end
