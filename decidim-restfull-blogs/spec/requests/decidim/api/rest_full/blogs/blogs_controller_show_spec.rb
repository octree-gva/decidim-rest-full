...
RSpec.describe Decidim::Api::RestFull::Blogs::BlogsController do
  path "/blogs/{id}" do
    get "Show post" do
      ...
      parameter name: "id", in: :path, schema: { type: :integer, description: "Post id" }
      ...
      describe_api_endpoint(
        ...
        response "200", "Blog Found" do
          ...
          context "when the post is alone" do
            run_test! do |example|
          ...
          context "when the post is the last one (published_at ASC)" do
            let(:order) { "published_at" }
          ...
          context "when the post is first one (published_at ASC)" do
            let(:order) { "published_at" }
            let(:order_direction) { "asc" }
            let!(:posts) do
              Array.new(3) { create(:post, component:, author: create(:user, :confirmed, organization:)) }.each_with_index do |post, index|
          ...
          context "when the second post in a collection of 3" do
            let(:order) { "published_at" }
            let(:order_direction) { "asc" }
            let!(:posts) do
              Array.new(3) { create(:post, component:, author: create(:user, :confirmed, organization:)) }.each_with_index do |post, index|
  ...
  describe "conditional GET" do
    ...
    let!(:participatory_process) { create(:participatory_process, organization:) }
    let!(:component) { create(:component, participatory_space: participatory_process, manifest_name: "blogs", published_at: Time.zone.now) }
    let!(:blog_post) { create(:post, component:, published_at: 2.days.ago) }
    let!(:api_client) do
      c = create(:api_client, organization:, scopes: ["blogs"])
      c.permissions = [c.permissions.build(permission: "blogs.read")]
      c.save!
      c
    end
    let!(:token) { create(:oauth_access_token, scopes: "blogs", resource_owner_id: nil, application: api_client) }

    before { host! organization.host }

    it "returns 304 when If-None-Match matches ResourceShowFingerprint" do
      path = "/api/rest_full/v#{Decidim::RestFull.major_minor_version}/blogs/#{blog_post.id}"
      qs = "?component_id=#{component.id}&space_id=#{participatory_process.id}&space_manifest=participatory_processes"
